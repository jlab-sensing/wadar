...
0.15-0.33ns, which is sufficient for sensing soil moisture~\cite{}. We
propose using the Walabot Developer UWB pulsed radar, which is readily
...
and~\cite{uwbThesis} shows that it is well suited to being mounted on drones or
autonomous robots for automated data collection. This would allow both
small-holder farmers and large commercial farms to benefit.
...
\section*{Future work}
EC  It
might also be possible to measure electrical conductivity (EC),
...
within the adademic EE and communications communities. This creates
oppotunities for cross-disciplinary work and opens up a rich set of
open problems with global impact. Using RF backscater to detect
...
and receiving at all times, while pulsed radars periodically trasmit a
short-duration pulse and listen for the relections to come back.
...
calulating the target distance, or \emph{range}. Frequency Modulated
Continuous Wave (FMCW) is a variant that is capable of calulating
...
Pulsed radars\footnote{Pulse radars are also sometimes called
  ...
  Doppler radars because they too use Doppler to measure spped.} can
