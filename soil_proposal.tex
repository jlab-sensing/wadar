...
moisture~\cite{smurf}. We propose using the Walabot Developer UWB
pulsed radar, which is readily commercially available and has a
bandwidth of 6.7Ghz. This corresponds to a ToF resolution of
0.15ns. This radar is potentially already integrable with an Android
smartphone\footnote{the Walabot DIY model is compatible with Android
  smartphones, but does not allow collection of raw samples. As per
  correspondence with their engineering team, they are planning to
  release an Android SDK for the developer in the future and may give
  us early access}, and~\cite{uwbThesis} shows that it is well suited
