...
portNoAll = 17:21; %[17 18 19 20 21]; %[7 8 9 10  14];  % port numbers where Radars are seen
...
for iRx = 1:nRx
    ...
    dopplerIndExpected = 25600;
    rangeBinConsiderAll = [16 12 12 12 13];
    radarIqCorrelated =  radarDataOversampleForMaxIdxAllRx{iRx};
    [~,timeShiftConsider] = max(abs(radarIqCorrelated(rangeBinConsiderAll(iRx),:)));
    radarDataConsider = radarDataOversampleForMaxIdxAllRx{iRx}(:,timeShiftConsider);
    figure(7); plot(abs(radarDataConsider)); hold on; plot(abs(radarDataOversampleForMaxIdxAllRx{iRx}(:,1))); hold off; title(sprintf('for COM%d',portNoAll(iRx)));
...
% % save limited data
save(sprintf('D:\\PhaseLimitedXethruData\\PhaseLimitedExpData%s.mat',fileExtension),'-v7.3','shiftRadTot','radarDataOversampleForMaxIdxAllRx');
