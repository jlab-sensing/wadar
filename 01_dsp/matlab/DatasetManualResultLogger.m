function DatasetManualResultLogger(localDataPath)
...
[peakDifference, SNRdB1, SNRdB2] = DatasetManualFramesProc(localDataPath, tag1Hz, tag2Hz, label.scansPerDatum);

for i = 1:length(peakDifference) % Removing obvious weird scans
    if abs(peakDifference(i) - median(peakDifference)) > 10
        peakDifference(i) = median(peakDifference);
    end
end
